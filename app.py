...
def _backup_cipher():
    ...
    key = os.environ.get("BACKUP_ENCRYPTION_KEY") or globals().get("BACKUP_ENCRYPTION_KEY", "")
    if not key:
...
def _gcs_bucket():
    ...
    bucket_name = os.environ.get("GCS_BUCKET_NAME") or globals().get("GCS_BUCKET_NAME", "")
    if not bucket_name:
...
def health():
    try:
        ...
        db_status = f"error: {e}"
    return jsonify({"status": "ok", "app": APP_NAME, "product_code": PRODUCT_CODE, "database": "postgresql", "db_status": db_status, "backup": {"gcs_bucket": bool(os.environ.get("GCS_BUCKET_NAME") or globals().get("GCS_BUCKET_NAME", "")), "encryption_key": bool(os.environ.get("BACKUP_ENCRYPTION_KEY") or globals().get("BACKUP_ENCRYPTION_KEY", ""))}})
...
def backup_upload():
    ...
    try:
        ...
        object_name = f"smartpay/{safe_key}/{safe_device}/{ts.replace(':','').replace('-','').replace('Z','')}_{_safe_object_part(filename)}.enc"
        bucket = _gcs_bucket()
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO cloud_backups(
                        license_key, device_id, product_code, firm_name, app_version, backup_time,
                        object_name, original_filename, backup_size, checksum_sha256, status
                    ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'active')
                    RETURNING id
                """, (license_key, device_id, product_code, firm_name, app_version, ts, object_name, filename, len(raw), checksum))
